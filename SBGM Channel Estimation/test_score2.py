...
contents    = torch.load(target_file,  map_location=torch.device('cpu'), weights_only=False)
...
for meta_idx, (spacing, pilot_alpha) in tqdm(enumerate(meta_params)):
    ...
    val_P = val_sample['P'].to(config.device)
    # Hermitian pilots
    val_P = torch.conj(torch.transpose(val_P, -1, -2))
    val_H_herm = val_sample['H_herm'].to(config.device)
    ...
    for snr_idx, local_noise in tqdm(enumerate(noise_range)):
        ...
        for step_idx in tqdm(range(val_config.model.num_classes)):
            ...
            labels = torch.ones(init_val_H.shape[0]).to(config.device) * step_idx
            labels = labels.long()
            ...
            # 1) Batched SVD
            U, S, Vh = torch.linalg.svd(forward, full_matrices=False)
            #    S: [5, r]

            Vh_H = Vh.conj().transpose(-2, -1)   # batched Hermitian transpose
            U_H  = U .conj().transpose(-2, -1)

            # 2) build the batched diagonal Σ: shape [5, r, r]
            Sigma = torch.diag_embed(S).to(forward.dtype)# Σ[i] is diag(σ₁,…,σ_r) for batch i

            # 3) identity [r, r] (will broadcast to [5, r, r])
            r = S.size(-1)
            I = torch.eye(r, device=forward_h.device, dtype=forward_h.dtype)

            # 4) elementwise multiply Σ with itself to get diag(σ_i²)
            Sigma2 = Sigma * Sigma        # [5, r, r]

            # 5) add local_noise * I
            inv_noise_plus_sigma2 = torch.linalg.inv((local_noise) * I + (current_sigma**2)*Sigma2)
            #    → [5, r, r]
            ...
            for inner_idx in range(val_config.sampling.steps_each):
                ...
                core      = Sigma.matmul(inv_noise_plus_sigma2)  # [5, r, r]
                pseudo_inv = Vh_H.matmul(core).matmul(U_H)       # [5, N, M]
                meas_grad  = pseudo_inv.matmul(y - forward @ current)  # [5, N]
                # Sample noise
